...
//!
//! Run: cargo test -p fqtl-rs --test sim_mediation_roundtrip -- --nocapture

use anyhow::Result;
use fqtl_rs::genotype::bed_writer::write_plink;
use fqtl_rs::genotype::GenotypeMatrix;
use fqtl_rs::simulation::{
...
use fqtl_rs::summary_stats::{compute_block_sumstats, compute_yty_diagonal};
...
fn test_sim_mediation_roundtrip() -> Result<()> {
    ...
    use fqtl_rs::genotype::{BedReader, GenomicRegion, GenotypeReader};
...
fn test_collider_selection_changes_gwas() -> Result<()> {
    ...
    let std_expr = fqtl_rs::simulation::standardize_dvector(&expressions[cond_gene.gene_idx]);
