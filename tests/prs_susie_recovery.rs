...
//!
//! Run: cargo test -p fqtl-rs --test prs_susie_recovery -- --nocapture

use anyhow::Result;
use fqtl_rs::candle_util::candle_core::Device;
use fqtl_rs::candle_util::sgvb::cavi_susie::{cavi_susie, CaviSusieParams};
use fqtl_rs::matrix_util::traits::{ConvertMatOps, MatOps, SampleOps};
use fqtl_rs::sgvb::{fit_block, fit_block_rss, BlockFitResult, FitConfig, ModelType, RssParams};
use fqtl_rs::simulation::sample_cell_type_genetic_effects;
use fqtl_rs::summary_stats::polygenic_score::{
    compute_all_polygenic_scores_ridge, compute_block_polygenic_scores_ridge,
};
use fqtl_rs::summary_stats::LdBlock;
...
fn test_prs_susie_recovers_causal_snps() -> Result<()> {
    ...
    let yhat_tensor =
        fqtl_rs::candle_util::candle_core::Tensor::from_slice(&yhat_col, (n,), &device)?;
    ...
    let sgvb_config = FitConfig {
        ...
        prior_type: fqtl_rs::sgvb::PriorType::Single,
        num_components: 10,
        num_sgvb_samples: 20,
        learning_rate: 0.01,
        num_iterations: 500,
...
fn run_rss_cavi(
    ...
    use fqtl_rs::candle_util::sgvb::cavi_susie::{cavi_susie, CaviSusieParams};
    use fqtl_rs::summary_stats::rss_svd::RssSvdNal;
    ...
    let y_tensor =
        fqtl_rs::candle_util::candle_core::Tensor::from_slice(&y_col, (y_tilde.nrows(),), device)?;
