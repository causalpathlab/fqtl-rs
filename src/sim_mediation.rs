...
use fqtl_rs::io::gene_annotations::{load_bed_annotations, load_gtf};
use fqtl_rs::matrix_util::common_io::mkdir_parent;
use fqtl_rs::matrix_util::dmatrix_util::subset_columns;
...
pub fn sim_mediation(args: &SimMediationArgs) -> Result<()> {
    ...
    if args.induce_collider_bias {
        ...
        for &gi in conditioned {
            ...
            let std_expr = fqtl_rs::simulation::standardize_dvector(&expressions[g]);
            liability += &std_expr;
        }
