...
use fqtl_rs::sgvb::refinement::{refine_high_pip_variants, RefinementInput, RefinementParams};
use fqtl_rs::sgvb::{
...
pub fn fit_sumstat_sgvb(args: &FitSumstatSgvbArgs) -> Result<()> {
    ...
    let fit_config = FitConfig {
        ...
        batch_size: args.batch_size.unwrap_or_else(|| {
            fqtl_rs::matrix_util::utils::default_block_size(input.zscores.nrows())
        }),
        prior_vars,
    ...
    fqtl_rs::summary_stats::common::report_top_hits(
        &globally_averaged,
        &input.blocks,
        &input.zscores,
        &input.geno,
